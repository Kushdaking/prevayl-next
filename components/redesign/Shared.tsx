...
export function Links({items}:{items:{href:string;title:string;body:string}[]}){return <div className="pvl-link-grid">{items.map((l,i)=><Link href={l.href} key={l.href} className="pvl-link-card"><span className="pvl-card-number">{String(i+1).padStart(2,"0")} <span aria-hidden="true">↗</span></span><span className="vx-icon"><VisualIcon name={/carrier/i.test(l.title)?"truck":/account|pricing|intelligence/i.test(l.title)?"chart":/compliance|pin/i.test(l.title)?"shield":/order|dealer|auction/i.test(l.title)?"document":/ai|bot/i.test(l.title)?"people":"pin"}/></span><MiniVisual kind={featureVisual(l.title)}/><h3>{l.title}</h3><p>{l.body}</p></Link>)}</div>}
